...
Fish.prototype.drawBody = function () {
  ...
  for (var i = -1; i < 2; i += 2) {
    ...
    this.ctx.bezierCurveTo(
      this.size * (14 / 15),
      i * this.size + (this.size / 30) * o + this.curv / 3,
      -this.size / 2,
      i * this.size + (this.size / 30) * o + this.curv / 2,
      -this.size * 2,
      (i * this.size) / 3 + (this.size / 15) * o + this.curv
    );
    this.ctx.bezierCurveTo(
      -this.size * 2.5,
...
Fish.prototype.collide = function (fish) {
  ...
  if (
    this.dying ||
    fish.dying ||
    distance(this, fish) > this.size * 5 + fish.size * 5
  ) {
    return false;
  }
...
Fish.prototype.physics = function () {
  ...
  if (this.dying) {
    ...
    if (!this.boost) {
      ...
      this.x += this.velocity[0] * Math.abs(Math.cos(this.dir)) * 2;
      this.y += this.velocity[1] * Math.abs(Math.sin(this.dir)) * 2;
    }
  }
